...
#define COMM_BUFFER_SIZE	4		/* SPI Communicate buffer size. */  //MUSS 4 SEIN!!!!!


#define TDM_SLAVE 			5		/* Can be 0 to 7 for TDM8 */
#define TDM_LRCLK_GEN		0		/* Enables Clock to generate TDM Clock */
#define DATASWITCH			0		/* 0 Microphone Data
									   1 Same data over and over
									   2 counter up to 0xffffff */
...
#define TC_WAVEFORM_FREQUENCY	24000 /* rate at witch Audio is sampled 48kHz */
#define TC_WAVEFORM_DUTY_CYCLE	2	/* duty cycle of 0.2% best practice between recognition an time it steals from whole slot*/
...
void SPI_Handler(void)
{
	...
	if(status & SPI_SR_NSSR) {
		...
		// transfer static test data (for debugging and testing)
		#if DATASWITCH == 1
		gs_uc_spi_s_tbuffer[2] = (uint8_t)(0xfc);
		gs_uc_spi_s_tbuffer[1] = (uint8_t)(0xfc);
		gs_uc_spi_s_tbuffer[0] = (uint8_t)(0x99);
		#endif
		
		// transfer a value counting from 0x0 up to 0xffffff (for testing an debugging)
		#if DATASWITCH == 2
		gs_uc_spi_s_tbuffer[2] = (uint8_t)(counterData >> 0);
		gs_uc_spi_s_tbuffer[1] = (uint8_t)(counterData >> 8);
		gs_uc_spi_s_tbuffer[0] = (uint8_t)(counterData >> 16);
		if (counterData == 0xffffff){
			counterData = 0;
		} else {
			counterData++;
		}
		...
		// counter to let LED0 blink regularly when data is transmitted
...
static void tc_waveform_initialize(void)
{
	...
	/*************************************
	 Set TDM Solt position
	 (up to TDM8 is implemented)
	 *************************************/

	// technically only ra event is needed 
	#if TDM_SLAVE == 0
	ra = 32; 
	rb = 32; 
	rc = 65; 
	...
	| TC_CMR_ACPC_NONE	// RC Compare Effect on TIOAx (p.559)
    ...
    // technically only the ra and rc events are needed
	#if TDM_SLAVE == 1
	ra = 32; 
	rb = 32; 
	rc = 65; 
