...
#define BUFFER_MULTI        2
//#define DEBUG


int32_t bufMic[NUM_OF_MIC][BUFFER_LENGTH*BUFFER_MULTI] = {{0}}; // global is the simple version
double mic0Analyse[1024*BUFFER_MULTI] = {0};
int32_t tempbufMic = 0;                             //Ermittlung höchster Pegel

void fastAddValue(int32_t value, int micNum){
    memcpy(bufMic[micNum], bufMic[micNum]+1, (BUFFER_LENGTH-1*BUFFER_MULTI)*NUM_OF_BYTES_VALUE);
    bufMic[micNum][BUFFER_LENGTH-1*BUFFER_MULTI] = value;
}
...
void getDataForSoundDetect(){   
    for (int i=0; i<1024*BUFFER_MULTI; i++){
...
//    
    ...
    float zeit, zeit_gesamt;
    clock_t start, ende, start_gesamt, ende_gesamt;
    start = clock();
    ...
    while (counter){
        updateValues(1024*BUFFER_MULTI,&pipe_filestream);
        detector.processFFT(mic0Analyse);
        getDataForSoundDetect();
        
        if(detector.checkForBang(mic0Analyse) && (blind < 1)){
            cout<<"Event erkannt"<<endl;
            start = clock();
            start_gesamt = clock();
            int result = direction.getAngle(bufMic[0],bufMic[1],bufMic[2],bufMic[3],bufMic[4],bufMic[5],BUFFER_LENGTH*BUFFER_MULTI);
            cout <<"Winkel des Events: "<<result<<"°"<<endl;
            ende = clock();
                //Ergebniss der Laufzeitmessung in Sekunden
            zeit = (float)(ende-start) / (float)CLOCKS_PER_SEC;
            cout<<"Dauer der Berechnung:"<<zeit<<endl;
            ...
            blind = 8;               //um ein Widerhall des Geräusches zu ignorieren
                if(result >= 320 || result <= 40){
                    ...
                    ende_gesamt = clock();
                    cout<<"Event war im Bildbereich"<<endl;
                }else{
                    cout<<"Event war NICHT im Bildbereich"<<endl;
                    ende_gesamt = clock();
                }
            zeit_gesamt = (float)(ende_gesamt-start_gesamt) / (float)CLOCKS_PER_SEC;
            cout<<"Dauer vom Event bis zum Bild:"<<zeit_gesamt<<endl;
            cout<<"---------------Neue Messung---------------"<<endl;
            }
/*
        for (int i=0; i<1024*BUFFER_MULTI; i++){
